...
function tradeActions(state: State, playerId: PlayerId): Action[] {
  if (!state.turn.hasRolled) return []; // bank trade only in the build phase
  const p = state.players[playerId]!;
  const out: Action[] = [];
...
function tradeRespond(s: State, playerId: PlayerId, accept: boolean): State {
  ...
  if (accept) {
    // re-validate at accept time: the offerer may have spent the offered cards since offering
